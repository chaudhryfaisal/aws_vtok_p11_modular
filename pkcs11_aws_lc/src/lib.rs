//! Complete PKCS#11 provider with AWS-LC cryptographic backend
//!
//! This crate provides a standalone PKCS#11 provider that uses AWS-LC
//! as the underlying cryptographic implementation. It can be used as
//! both a library and a cdylib for direct use by PKCS#11 applications.
...
// Our AWS-LC backend implementation
pub mod backend;

// Provider integration
mod provider;

use backend::AwsLcBackend;
use provider::AwsLcProvider;

/// Global provider instance
lazy_static! {
    static ref PROVIDER: Arc<Mutex<Option<AwsLcProvider>>> = Arc::new(Mutex::new(None));
}

static INIT: Once = Once::new();

/// Initialize the PKCS#11 provider with AWS-LC backend
fn ensure_provider_initialized() -> Result<(), vtok_p11::Error> {
    INIT.call_once(|| {
        env_logger::init();
        
        match AwsLcBackend::new() {
            Ok(backend) => {
                let provider = AwsLcProvider::new(backend);
                if let Ok(mut guard) = PROVIDER.lock() {
                    *guard = Some(provider);
                    info!("AWS-LC PKCS#11 provider initialized successfully");
                } else {
                    error!("Failed to acquire provider lock during initialization");
                }
            }
            Err(e) => {
                error!("Failed to create AWS-LC backend: {:?}", e);
            }
        }
    });
...
fn get_provider() -> Result<Arc<Mutex<Option<AwsLcProvider>>>, vtok_p11::Error> {
    ensure_provider_initialized()?;
    Ok(PROVIDER.clone())
}

// Export all PKCS#11 C API functions
// These will delegate to the vtok_p11 implementation with our AWS-LC backend
