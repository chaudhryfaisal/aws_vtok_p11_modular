...
macro_rules! lock_device {
    ($guard:ident, $device:ident) => {
        ...
        let $device = $guard.as_ref().unwrap().as_ref();
    };
}
...
macro_rules! lock_device_mut {
    ($guard:ident, $device:ident) => {
        ...
        let $device = $guard.as_mut().unwrap().as_mut();
    };
}
...
macro_rules! lock_session {
    ($handle:expr, $session_guard:ident, $session:ident) => {
        lock_device!(guard, device);
        let sess_arc = match device.session($handle) {
            Some(sess) => sess,
            None => return crate::pkcs11::CKR_SESSION_HANDLE_INVALID,
        };
        let $session_guard = sess_arc.lock().unwrap();
...
macro_rules! lock_session_mut {
    ($handle:expr, $session_guard:ident, $session:ident) => {
        ...
        let sess_arc = match device.session_mut($handle) {
            Some(sess) => sess,
            None => return crate::pkcs11::CKR_SESSION_HANDLE_INVALID,
        };
        let mut $session_guard = sess_arc.lock().unwrap();
        let $session: &mut (dyn crate::backend::SessionTrait + Send + Sync) = &mut *$session_guard;
    };
}
...
pub fn initialize<B: VtokBackend + 'static>(backend: B) -> pkcs11::CK_RV {
    let mut maybe_device = data::DEVICE.lock().unwrap();
    if maybe_device.is_some() {
        return pkcs11::CKR_CRYPTOKI_ALREADY_INITIALIZED;
    }

    match Device::new(backend) {
        Ok(device) => {
            maybe_device.replace(Box::new(device));
            pkcs11::CKR_OK
        }
        Err(e) => e.into(),
    }
...
pub extern "C" fn C_Initialize(pInitArgs: pkcs11::CK_VOID_PTR) -> pkcs11::CK_RV {
    ...
    // The backend is not initialized here anymore. The final application that links this
    // library is responsible for calling `initialize` with a concrete backend.
