...
pub struct Token<B: VtokBackend> {
    slot_id: pkcs11::CK_SLOT_ID,
    label: String,
    sessions: HashMap<pkcs11::CK_SESSION_HANDLE, Arc<Mutex<Session<B>>>>,
    db: Option<Db>,
    user_login: bool,
    expiry_ts: u64,
    backend: Arc<B>,
...
impl<B: VtokBackend> Token<B> {
    pub fn from_config(
        ...
        Ok(Self {
            ...
            db: Some(Db::from_token_config(token_config, backend.as_ref()).map_err(Error::DbLoad)?),
            user_login: false,
