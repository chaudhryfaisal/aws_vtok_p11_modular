...
pub struct Slot<B: VtokBackend> {
    _id: pkcs11::CK_SLOT_ID,
    token: Option<Token<B>>,
}

impl<B: VtokBackend> Slot<B> {
    ...
    pub fn new_with_token(id: pkcs11::CK_SLOT_ID, token: Token<B>) -> Self {
        Self {
            _id: id,
            token: Some(token),
        }
    ...
    pub fn has_token(&self) -> bool {
        self.token
            .as_ref()
            .filter(|tok| !tok.has_expired())
            .is_some()
    }

    pub fn token(&self) -> Option<&Token<B>> {
        self.token.as_ref().filter(|tok| !tok.has_expired())
    }

    pub fn token_mut(&mut self) -> Option<&mut Token<B>> {
        self.token.as_mut().filter(|tok| !tok.has_expired())
    }

    pub fn token_unchecked_mut(&mut self) -> Option<&mut Token<B>> {
        self.token.as_mut()
    }
}
